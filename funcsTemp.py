...
def RV_model(t, period, ttran, ecosomega, esinomega, K):
	"""
	...
	RV = K * (e * np.cos(omega) + np.cos(fanom + omega))
...
def loglikelihood(p, t, RV, RVerr, chisQ=False):
	...
	for ii in range(0, numSpec):
		models.append(RV_model(t[ii], period, ttran, ecosomega, esinomega, K))
	...
	for ii in range(0, numSpec):
		totchisq += np.sum((RV[ii]-models[ii]-gammas[ii])**2. / ( 2. * (RVerr[ii]**2. + jitterSqrd[ii]) ))
		loglikelihood += -np.sum( 
			(RV[ii]-models[ii]-gammas[ii])**2. / ( 2. * (RVerr[ii]**2. + jitterSqrd[ii]) ) +
