...
def runMCMC(p, t, rv, rvErr, outfile, niter=10000, nwalkers=50):
	...
	startlocs = [p + initrange(p, 6) * np.random.randn(ndim) for i in np.arange(nwalkers)]
	...
	for result in sampler.sample(startlocs, iterations = niter, storechain = False, threads = 2):
		pos = result[0]
...
t, rv, rvErr = readObservations('./HD102509/HD102509_yahalomiMarch21.orb', True)
...
#run MCMC
print runMCMC(p, t, rv, rvErr, './HD102509/chain_100000_gammas_negJit.txt', niter = 100000)
